...
biết chơi cờ mang tên ``Mechanical Turk''. Đây là một thiết bị tinh vi với
...
\begin{itemize}
    ...
    như tách biệt này liệu có chia sẻ cùng cấu trúc toán học không?
...
dẹp. Đây là một hiện tượng được gọi là \textit{khai thác phần thưởng}.
...
\textbf{Mô hình Nền tảng Hành vi:} Tương tự như GPT là mô hình nền tảng cho ngôn ngữ, các Mô hình Nền tảng Hành vi (được học thông qua Học Tăng cường không có phần thưởng) có thể đóng vai trò mô hình nền tảng cho hành động, một model dùng được cho nhiều tác vụ robot khác nhau mà không cần huấn luyện lại.
...
AlphaProof giải bài thi toán Olympicl. Các hệ thống này đều dùng Học Tăng cường để tìm giải pháp mà không có dữ liệu tương ứng từ trước.
...
là chúng ghi nhớ dữ liệu đã thấy, mà là chúng \textit{tổng quát hóa}, tức tạo ra output đúng trên những input chưa bao giờ thấy trong training.
...
\begin{figure}[H]
    ...
    Mục tiêu tác nhân là học policy $\pi$ tối đa hóa tổng phần thưởng
    tích lũy.}
    \label{fig:rl-loop}
\end{figure}
...
\subsubsection{Policy và Mục tiêu Học Tăng cường}
 
\textbf{Policy} $\pi$ là chiến lược ra quyết định của tác nhân, hay một
...
Một \textbf{trajectory} $\tau = (s_0, a_0, s_1, a_1, \ldots)$
là chuỗi trạng thái và hành động theo thời gian, về bản chất là một
``ván chơi'' hoàn chỉnh. \textbf{Return chiết khấu} $R(\tau)$ là
...
\textbf{Mục tiêu Học Tăng cường} là tìm policy $\pi$ tối đa hóa return kỳ vọng, tức phần thưởng tích lũy kỳ vọng trên tất cả các trajectory có thể
xảy ra:
...
policy $\pi$ tương tác với môi trường: tại mỗi bước $t$, tác nhân chọn
...
thuộc vào tham số đang học. Trong HTc, trajectory được sinh ra bởi
...
đến return cao nhất?''. Đây là vai trò của \textbf{hàm giá trị}.
...
Q-Function xuất hiện xuyên suốt tutorial: trong diffusion policy
...
và thực hiện leo dốc gradient. Định lý Policy Gradient cho kết quả:
...
\textbf{hàm điểm số} của policy. Đây là cầu nối kỹ thuật quan trọng
...
dẫn quá trình sinh mẫu. Hai bài toán policy optimization và sinh mẫu từ
...
$\nabla_x \log p_t(x)$ và hàm điểm số của policy gradient
$\nabla_\theta \log \pi_\theta(a \mid s)$ là cùng một cấu trúc toán
học -- gradient của log-xác suất theo biến cần tối ưu. Đây là lý
do mô hình khuếch tán và policy gradient có thể được tích hợp tự nhiên
vào nhau, như được minh họa qua DDPO và Diffusion-DICE ở Phần III.

Trong Học Tăng cường, mô hình khuếch tán được dùng làm \textbf{diffusion policy}:
thay vì Gaussian policy đơn giản chỉ biểu diễn một đỉnh, tác nhân dùng
một mô hình khuếch tán để biểu diễn phân phối hành động phức tạp, đa
đỉnh, điều này quan trọng khi tập dữ liệu chứa nhiều chiến lược khác nhau.
...
\textit{hàm khả năng của trạng thái tương lai},tức thước đo kế tiếp, kết hợp với cấu trúc Bellman để tận dụng off-policy data. Đây
...
\paragraph{Transformer Quyết định: mô hình tự hồi quy làm policy.}
...
cập nhật Bellman, mô hình coi một trajectory trong Học Tăng cường -- chuỗi
$(R_0, s_0, a_0, R_1, s_1, a_1, \ldots)$ -- như một chuỗi token và
học sinh hành động có điều kiện trên \textit{return-còn-lại} (tổng
phần thưởng kỳ vọng từ bước hiện tại trở đi) và lịch sử trajectory.

Ý tưởng then chốt: nếu ta điều kiện hóa mô hình trên ``return mục tiêu
...
của việc dùng mô hình tạo sinh làm policy trong Học Tăng cường.
...
với policy ``luôn đi thẳng về phía đông''. Thước đo kế tiếp
...
Kết nối giữa thước đo kế tiếp và Q-Function rất trực tiếp. Nếu có
...
thể tính Q-Function cho \textbf{bất kỳ hàm thưởng nào} mà không
...
off-policy bằng học hiệu chỉnh thời gian, tương tự như cách ta ước
lượng Q-Function. Đây là lý do thước đo kế tiếp có thể được học hiệu
...
các biểu diễn tiến-lùi học cách phân tích thước đo kế tiếp để cho phép tính policy tối ưu cho bất kỳ hàm thưởng nào
...
trạng thái-hành động mà policy ghé thăm (tổng quát hóa thước đo kế tiếp sang
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.32\textwidth} p{0.32\textwidth} p{0.32\textwidth}}
        ...
        \textbf{Ý nghĩa chung} \\
        \midrule
        Policy $\pi(a \mid s)$ & Mô hình tạo sinh $p(x)$ & Phân phối cần học \\
        Hàm điểm số $\nabla_\theta \log \pi(a \mid s)$ & Hàm điểm số $\nabla_x \log p(x)$ & Gradient của log-xác suất \\
        Return $R(\tau)$ & Log-hàm khả năng $\log p(x)$ & Tín hiệu học \\
        Trajectory $\tau$ & Biến tiềm ẩn $z$ & Biến tiềm ẩn \\
        Policy Gradient & Ước lượng hàm điểm số & Cách tối ưu phân phối \\
        Q-Function $Q(s,a)$ & Tỉ lệ hàm khả năng $\frac{p^\pi(s_f|s,a)}{p(s_f)}$ & Ước lượng xác suất tương lai \\
        Thước đo chiếm dụng $d^\pi(s,a)$ & Phân phối dữ liệu $p(x)$ & Phân phối muốn khớp \\
        Phương trình Bellman & Tính chất Markov & Cấu trúc đệ quy \\
        Nhãn kỹ năng $z$ & Mã tiềm ẩn $z$ trong VAE & Điều khiển hành vi \\
        \bottomrule
    \end{tabular}
    \normalsize
\end{table}
