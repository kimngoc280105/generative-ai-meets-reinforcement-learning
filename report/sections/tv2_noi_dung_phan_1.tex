...
\begin{figure}[H]
    ...
    \caption{Pipeline model-based RL với mô hình thế giới: generative model được huấn luyện để học dynamics từ transition data, sau đó hỗ trợ học chính sách. Nguồn hình: Figure 3 trong \cite{eysenbach2025tutorial}.}
...
\begin{figure}[H]
    ...
    \caption{Toycase 2D bandit của Diffusion-DICE: action trong bộ dữ liệu nằm trong vùng hình khuyên; true reward cao ở outer circle; learned reward overestimate inner circle; Diffusion-DICE giảm khai thác lỗi tốt hơn các hướng guide-only hoặc select-only. Nguồn hình: Figure 7 trong \cite{eysenbach2025tutorial}.}
...
Part II khảo sát khả năng đặc tả một lớp bài toán RL bằng desired observations hoặc goal data. Cách này chuyển một phần bài toán từ reward engineering sang khớp phân phối trên outcome~\cite{eysenbach2025tutorial}.
...
Ở mức on-policy, f-divergence gradient cung cấp một cầu nối tự nhiên giữa khớp phân phối goal và gradient chính sách. Trong offline setting, yêu cầu học từ bộ dữ liệu cố định dẫn tới các formulation dual và DICE được trình bày tiếp theo~\cite{eysenbach2025tutorial}.
...
Estimator trên còn mang tính on-policy, nên đòi hỏi quỹ đạo từ chính sách hiện tại và có thể kém hiệu quả dữ liệu~\cite{eysenbach2025tutorial}.
...
Những tác vụ này cần mô hình hóa phân phối trên quỹ đạo hoặc outcome có cấu trúc, thay vì chỉ thay reward bằng một ảnh đích~\cite{eysenbach2025tutorial}.
