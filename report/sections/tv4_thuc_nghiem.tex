...
\begin{itemize}
    \item \textbf{QGPO} bị hút vào tâm OOD $(0,0)$ vì đạo hàm
    ...
    \item \textbf{Diffusion-DICE} sinh mẫu tập trung tại hai đỉnh
    phần thưởng thực tế trên vành ngoài ($\theta \approx 0$ và $\theta \approx
    \pi$), gần như hoàn toàn tránh xa bẫy OOD ở tâm. Cơ chế IGL học trọng số
    in-sample $w^*(a)$ từ chính dữ liệu trong $\mathcal{D}$, bảo đảm guidance
    không kéo chính sách ra ngoài support dữ liệu.
\end{itemize}
...
\textbf{Phân tích kết quả}: 
\begin{itemize}
    \item Trong kịch bản phân phối rời rạc này, cả IDQL và Diffusion-DICE đều thể hiện khả năng mô hình hóa đa phương thức ở mức độ tương đương, với hành động sinh ra tập trung chủ yếu tại 4 cụm Gaussian đúng vị trí. Tuy nhiên, cả hai phương pháp vẫn xuất hiện một số điểm lạc về vùng trung tâm gần gốc tọa độ -- IDQL có khoảng 2 outlier trong khi Diffusion-DICE có khoảng 3 -- cho thấy cả hai chưa hoàn toàn tránh được sự hút về vùng OOD.
    \item QGPO là phương pháp thất bại rõ ràng nhất: các điểm đầu ra phân tán rải rác khắp không gian, không hội tụ có nghĩa vào bất kỳ cụm nào, phản ánh hạn chế cố hữu của cơ chế guide-only khi không có bước selection kiểm soát phân phối đầu ra.
\end{itemize}
